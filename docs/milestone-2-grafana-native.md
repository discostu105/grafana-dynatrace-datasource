...
| Req  | Status     | Notes                                                                                                                                                                                                                                                                                                                                                                   |
| ---- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| R2.1 | ✅ Done    | `plugin.json` declares `"logs": true`. `queryType: 'logs'` selector in `QueryEditor.tsx`; backend `recordsToLogFrame` emits `VisTypeLogs` with `time`/`body`/`level`/`labels`. Log volume histogram via `getSupplementaryQuery` + `buildLogsVolumeQuery` (`logsHooks.ts`). `DerivedFieldsEditor` configures regex-based body→URL links applied in `applyDerivedFields`. |
| R2.2 | ⚠️ Partial | `plugin.json` declares `"annotations": true` so Grafana's standard "use query result" path works — but there is **no dedicated annotation editor** with the `time / title / text` column-name inputs called for in this milestone, and no example DQL placeholder. Users must hand-craft the column names today.                                                        |
| R2.3 | ✅ Done    | `"alerting": true` set. Macros expand server-side in `pkg/macros`, so the alert path gets the same substitutions as panels. README's macros table documents this; a worked alert-rule example is not yet in the README.                                                                                                                                                 |
| R2.4 | ✅ Done    | `metricFindQuery` implemented on `DataSource`: runs the DQL via the regular backend path, reduces to `{text, value}` with the spec's one-column / two-column-named rules. Used internally by `getTagValues`.                                                                                                                                                            |
| R2.5 | ✅ Done    | `getTagKeys` returns curated keys merged with Grail autocomplete; `getTagValues` runs `summarize by:{key}` (capped 1000). `$__adhocFilters` macro substitutes a `                                                                                                                                                                                                       | filter ...`clause;`applyAdhocFilters` also auto-appends if the macro is absent. |
| R2.6 | ✅ Done    | `provisioning/datasources/datasources.yml` ships `${DT_TENANT_URL}` / `${DT_TOKEN}`; `docker-compose.yaml` forwards them. `docker compose up` brings up Grafana with the datasource pre-configured after the user sets the two env vars.                                                                                                                                |
| R2.7 | ⚠️ Partial | `tests/configEditor.spec.ts`, `tests/queryEditor.spec.ts`, `tests/macros.spec.ts` exist. Missing specs: Save & test on invalid token, logs visualization render, annotation rows, variable-query dropdown population. CI matrix against pinned minimum Grafana is not yet documented.                                                                                   |

**Overall:** Milestone 2 substantially done. Two gaps remain — dedicated annotations editor (R2.2) and the wider e2e suite (R2.7).
